...
export class Renderer {
  ...
  private renderBackground(ppu: Ppu): void {
    ...
    for (let y = 0; y < 240; y++) {
      for (let x = 0; x < 256; x++) {
        // Scroll com wrap entre duas nametables (simplificado)
        const pixelX = (x + scrollX) & 0x1FF; // 0..511
        const pixelY = (y + scrollY) & 0x1DF; // 0..479

        const tileX = (pixelX >> 3) & 0x1F;   // 0..31
        const tileY = (pixelY >> 3) & 0x1F;   // 0..31
        ...
        const tileRow = pixelY & 7;
        const tileCol = pixelX & 7;

        // Fetch de pattern (2bpp)
        const tileAddr = patternBase + (tileIndex * 16);
        const low  = ppu.readPpuMemory(tileAddr + tileRow);
        const high = ppu.readPpuMemory(tileAddr + tileRow + 8);
        const bit0 = (low  >> (7 - tileCol)) & 1;
        const bit1 = (high >> (7 - tileCol)) & 1;
